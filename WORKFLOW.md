...
## 流れの全体像：Claude Code で書く → チャットで確認 → OK なら画像 → Wix 投稿

```
1. Claude Code で記事を書いてもらう
   （トピックは topics/stock.md の上から1つ。下の「Claude Code 用 依頼文」をコピペして渡す）

2. Claude Code がチャット上に記事全文を表示する
   → ファイルを開かなくても、このチャット画面で記事を読んで確認できる。
...
→ AI がトピックを選び、記事本文（＋必要なら図解構成案）を出力する。**記事全文はチャット上にそのまま表示されるので、ファイルを開かずにここで読んで確認できる。「OK」または修正指示を伝える。**
...
あと、この記事用のアイキャッチ画像（と図解が必要なら図解画像）の「画像生成用プロンプト」も書いて。そのプロンプトはあとで ChatGPT に渡して画像を作る想定なので、ChatGPT にそのまま貼り付けて使える形で出して。
書き終わったら、記事全文をこのチャット上に表示して（ファイルを開かなくてもここで読んで確認できるように）。そのあと一度止まって、OKかどうか確認させて。
```
