"""10/25（日）花園日曜市 出店告知ブログ2本（あすなる愛媛・NLP island）をWixに下書き保存する。
...
D = os.path.join(os.path.dirname(__file__), "..", "drafts", "hanazono_2026-10")
...
def body_asunaru():
    n = [
        ...
        p("秋もぐっと深まって、花園通りのお散歩が気持ちいい季節になりましたね🍂"), sp(),
        p("今日は、うれしい告知のお知らせです！9月は台風で中止になってしまったので、2か月ぶりの出店です。"), sp(),
        p("10月25日（日）、花園通りで開催される花園日曜市に、今月もあすなる愛媛の「ワンコインカウンセリング」のテントを出します。"),
        sp(), "PHOTO",
    ]
    n += section_heading("10月25日（日）花園日曜市 出店内容")
    n += [sp()] + event_info()
    ...
    n += [
        ...
        p("テントで「実は相談所も気になっていて」と声をかけてくださったら、そのままゆっくりご説明しますね。"), sp(),
...
def body_nlp():
    n = [
        ...
        p("秋もぐっと深まり、朝晩の風がひんやり心地よくなってきましたね🍂"), sp(),
        p("そんな10月25日（日）、松山市駅前の花園通りで開催される松山花園日曜市に、今月も「ワンコインカウンセリング」として出店いたします🌿 9月は台風で中止になってしまったので、2か月ぶりの出店です。"),
        sp(), "PHOTO",
    ]
    n += section_heading("10月25日（日）松山花園日曜市 出店内容")
    ...
    n += [
        ...
        p("実は私自身、最近のモーニングジャーナルで「どうせ考えるなら、前向きで明るいほうを考えよう」と改めて決めたところです。考え方の癖は、いくつになっても、今日からでも変えていけます😊"),
    ]
    ...
    n += [
        sp(), p("10月25日（日）10時〜15時、松山市駅前の花園通りでお待ちしています。テント名は「ワンコインカウンセリング」です。"), sp(),
...
CTA_ASUNARU = "https://www.asunaru.jp/soudan?utm_source=blog&utm_medium=referral&utm_campaign=hanazono_2026-10"
...
SITES = {
    "asunaru": {
        ...
        "title": "10月25日（日）花園日曜市に出店します！ワンコイン婚活カウンセリング、予約なしで気軽にどうぞ",
        "excerpt": "10月25日（日）10時〜15時、松山市駅前の花園通りで開かれる花園日曜市に「ワンコインカウンセリング」のテントを出します。500円・20分・予約不要。婚活や恋愛、人間関係のことを、プロの心理カウンセラーで仲人の中嶋美知に気軽に話してみませんか。",
    ...
    "nlp": {
        ...
        "title": "10月25日（日）松山花園日曜市に出店します｜気持ちを少し話せるワンコインカウンセリング",
        "excerpt": "10月25日（日）10時〜15時、松山市駅前の花園通りで開かれる松山花園日曜市に「ワンコインカウンセリング」で出店します。500円・20分・予約不要。人間関係や日々のモヤモヤを、公認心理師・NLPトレーナーに気軽に話してみませんか。",
...
def update(key, did):
    """既存の下書き（9/27版）を10/25版の内容で上書きする。"""
    cfg = SITES[key]
    cover = upload(cfg, cfg["cover"])
    nodes = [photo_node(cover["id"], 1536, 1024, cfg["caption"]) if x == "PHOTO" else x for x in cfg["body"]()]
    body = {"draftPost": {
        "title": cfg["title"], "excerpt": cfg["excerpt"],
        "richContent": {"nodes": nodes, "metadata": {"version": 1}},
        "media": {"wixMedia": {"image": {"id": cover["id"], "url": cover["url"], "width": 1536, "height": 1024}},
                  "displayed": True, "custom": True},
        "seoData": {"tags": [{"type": "title", "children": cfg["title"]},
                             {"type": "meta", "props": {"name": "description", "content": cfg["excerpt"]}}],
                    "settings": {"preventAutoRedirect": False, "keywords": [{"term": cfg["focus_keyword"], "isMain": True}]}},
    }, "fieldMask": "title,excerpt,richContent,media,seoData"}
    r = requests.patch(f"{WIX_BASE}/blog/v3/draft-posts/{did}", headers=headers(cfg["site_id"]), json=body, timeout=30)
    print(key, "上書き:", "完了" if r.ok else f"失敗 {r.status_code} {r.text[:300]}")
